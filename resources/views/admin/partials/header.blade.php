  ...
  <div
    ...
    <div
      ...
      <div class="hidden lg:block">
        <!-- ✅ Welcome message với dữ liệu động -->
        @if(isset($userData) && Auth::check())
        <div class="mb-2 text-sm text-gray-600 dark:text-gray-400">
          Xin chào, <strong>{{ $userData['role_name'] ?? 'User' }}</strong> {{ $userData['name'] ?? 'Guest' }}!
        </div>
        @endif
        ...
        <div
          ...
          <div
            ...
            <ul class="custom-scrollbar flex h-auto flex-col overflow-y-auto">
              <!-- Notification items giữ nguyên như cũ -->
              <li>
                <a
                  ...
                  <span class="block">
                    <span
                      ...
                      <span class="font-medium text-gray-800 dark:text-white/90">Bệnh nhân mới</span>
                      đã đăng ký khám
                      <span class="font-medium text-gray-800 dark:text-white/90">Khoa nhi</span>
                    </span>
      ...
      <div
        ...
        <a
          ...
          <span class="mr-3 h-11 w-11 overflow-hidden rounded-full">
            @if(!empty($userData['avatar'] ?? null))
            <img src="{{ $userData['avatar'] }}" alt="{{ $userData['name'] ?? 'User' }}" />
            @else
            <img src="https://www.gravatar.com/avatar/{{ md5(strtolower(trim($userData['email'] ?? 'guest@example.com'))) }}?d=mp&s=44"
        ...
        <!-- ✅ Dropdown Start với dữ liệu động -->
        <div
          x-show="dropdownOpen"
          class="shadow-theme-lg dark:bg-gray-dark absolute right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 dark:border-gray-800">
          <!-- ✅ User info với dữ liệu động -->
          <div>
            <span
            ...
            @if(!empty($userData['role_name'] ?? null))
            <span
              class="text-theme-xs mt-0.5 block text-blue-600 dark:text-blue-400">
              {{ $userData['role_name'] }}
            </span>
            @endif
            @if(!empty($userData['department'] ?? null))
            <span
              class="text-theme-xs mt-0.5 block text-gray-500 dark:text-gray-400">
              {{ $userData['department'] }}
            </span>
          ...
          <!-- ✅ Logout form với proper action -->
          <form method="POST" action="{{ route('logout') }}" class="mt-3">
            @csrf
            <button
              type="submit"
              class="group text-theme-sm w-full flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
              <svg
