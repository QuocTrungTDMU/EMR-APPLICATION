...
(function () {
    ...
    class MedikNotificationSystem {
        ...
        async loadNotificationDetail(notificationId) {
            try {
                ...
                if (data.success) {
                    ...
                    throw new Error(data.message || 'Failed to load notification detail');
                }
            } catch (error) {
        ...
        async markAllAsRead() {
            try {
                ...
                if (data.success) {
                    ...
                    this.showToast('success', 'Thành công', `Đã đánh dấu ${data.data.updated_count || 'tất cả'} thông báo là đã đọc`);
                    return data.data;
                } else {
                ...
                this.showToast('error', 'Lỗi', 'Không thể đánh dấu tất cả thông báo là đã đọc');
                throw error;
            }
        }
        ...
        setupDropdown() {
            ...
            const markAllReadBtn = notificationDropdown.querySelector('#markAllReadBtn');
            if (markAllReadBtn) {
        ...
        renderNotifications() {
            ...
            container.innerHTML = this.notifications.map(notification => `
                <div class="px-6 py-4 border-b border-gray-100 hover:bg-blue-50 transition-colors notification-item cursor-pointer" 
                     ...
                     onclick="window.notificationSystem.viewDetail(${notification.id})">
                    <div class="text-xs text-gray-500 font-medium mb-2">${notification.formatedCreatedDate || ''}</div>
                    <div class="flex items-start space-x-3">
                        <div class="flex-shrink-0">
                            <div class="w-10 h-10 ${notification.is_read ? 'bg-green-100' : 'bg-red-100'} rounded-full flex items-center justify-center">
                                <div class="w-2 h-2 ${notification.icon_class || 'bg-blue-500'} rounded-full"></div>
                            </div>
                        </div>
                        <div class="flex-1 min-w-0">
                            <h4 class="text-sm ${notification.title_class || 'font-normal'} text-gray-900 mb-1">${notification.title}</h4>
                            <p class="text-sm text-gray-600 mb-2 line-clamp-2">${notification.body}</p>
        ...
        updateBadges() {
            ...
            const badges = document.querySelectorAll('#notificationBadge, #headerBadge');
        ...
        async viewDetail(notificationId) {
            ...
            try {
                const notification = await this.loadNotificationDetail(notificationId);
        ...
        displayNotificationDetail(notification) {
            ...
            if (typeElement) {
                ...
                typeElement.className = `inline-block px-3 py-1 rounded-full text-xs font-medium ${typeClasses[notification.type] || 'bg-gray-100 text-gray-800'}`;
                typeElement.textContent = typeLabels[notification.type] || '📄 Thông báo';
            }
            ...
            const additionalData = document.getElementById('modalAdditionalData');
            const dataContent = document.getElementById('modalDataContent');

            if (additionalData && dataContent) {
                if (notification.additional_data && Object.keys(notification.additional_data).length > 0) {
                    additionalData.classList.remove('hidden');

                    const dataHtml = Object.entries(notification.additional_data).map(([key, value]) => {
            ...
            if (markAsReadBtn) {
                if (!notification.is_read) {
                    ...
                    markAsReadBtn.onclick = () => this.markAsReadFromModal(notification.id);
                } else {
        ...
        async markAsReadFromModal(notificationId) {
            try {
                ...
                this.showToast('success', 'Thành công', 'Đã đánh dấu thông báo là đã đọc');
            } catch (error) {
        ...
        showToast(type, title, message, duration = 4000) {
            ...
            toast.className = `fixed top-4 right-4 z-50 bg-white shadow-lg rounded-lg border-l-4 ${this.getToastBorderColor(type)} p-4 max-w-sm transform transition-all duration-300 translate-x-full opacity-0`;

            toast.innerHTML = `
                <div class="flex">
                    <div class="flex-shrink-0">
                        <div class="flex items-center justify-center w-6 h-6 rounded-full ${this.getToastIconBg(type)}">
    ...
    window.showSuccessToast = function (title, message, duration) {
        if (window.notificationSystem) {
            window.notificationSystem.showToast('success', title, message, duration);
        }
    };

    window.showErrorToast = function (title, message, duration) {
        if (window.notificationSystem) {
            window.notificationSystem.showToast('error', title, message, duration);
        }
    };

    window.showWarningToast = function (title, message, duration) {
        if (window.notificationSystem) {
            window.notificationSystem.showToast('warning', title, message, duration);
        }
    };

    window.showInfoToast = function (title, message, duration) {
        if (window.notificationSystem) {
            window.notificationSystem.showToast('info', title, message, duration);
        }
    };
